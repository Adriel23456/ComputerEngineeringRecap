...
void PE2CPUView::ensureLoaded() {
    if (!m_tex) {
        ...
        if (!m_tex) std::cout << "[PE2CPUView] No se pudo cargar textura: " << fullPath << "\n";
        m_viewer.setZoomEnabled(false);
        m_viewer.setLockedVisibleFraction(0.80f);
        m_viewer.setMinVisibleFraction(0.50f);
    }
}
void PE2CPUView::setLabel(size_t i, const std::string& t) { if (i < m_labels.size()) m_labels[i] = t; }
void PE2CPUView::setLabels(const std::array<std::string, 5>& L) { m_labels = L; }

void PE2CPUView::render() {
    ensureLoaded();
    if (!m_tex) { ImGui::TextWrapped("No se pudo cargar 'Assets/CPU_TLP/CPU_Pipeline.png'."); return; }

    m_viewer.renderWithOverlay(*m_tex, "##PE2CPU_Viewer",
