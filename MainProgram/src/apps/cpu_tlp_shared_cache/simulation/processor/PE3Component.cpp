...
namespace cpu_tlp {
    ...
    PE3Component::RegisterFile::RegisterFile() {
        reset();
    }

    void PE3Component::RegisterFile::reset() {
    ...
    uint64_t PE3Component::RegisterFile::read(uint8_t addr) const {
        if (addr >= 12) return 0ULL;
        return regs[addr];
    }

    void PE3Component::RegisterFile::write(uint8_t addr, uint64_t value, bool we) {
        if (!we) return;
        if (addr >= 12) return;
    ...
    PE3Component::ALU::Result PE3Component::ALU::execute(uint8_t control, uint64_t A, uint64_t B, uint8_t flagsIn) {
        Result res;
        res.value = 0;
        res.flags = flagsIn;
    ...
    PE3Component::ControlUnit::Signals PE3Component::ControlUnit::decode(uint8_t opcode) {
        Signals sig;
    ...
    PE3Component::HazardUnit::Outputs
        PE3Component::HazardUnit::detect(
            bool INS_READY,
    ...
    uint64_t PE3Component::extendImmediate(uint32_t imm, bool dataType) {
        if (dataType == 0) {
    ...
    bool PE3Component::evaluateBranchCondition(uint8_t branchOp, uint8_t flags) {
        bool N = (flags & 0x8) != 0;
        bool Z = (flags & 0x4) != 0;
        bool C = (flags & 0x2) != 0;
        bool V = (flags & 0x1) != 0;
    ...
    PE3Component::PE3Component(int pe_id)
        : m_pe_id(pe_id)
    ...
    PE3Component::~PE3Component() {
        shutdown();
    }

    bool PE3Component::initialize(std::shared_ptr<CPUSystemSharedData> sharedData) {
        if (m_isRunning.load()) {
            return false;
        }
        ...
        // Lanzar hilo
        m_executionThread = std::make_unique<std::thread>(&PE3Component::threadMain, this);

        std::cout << "[PE" << m_pe_id << "] Initialized successfully\n";
        return true;
    }

    void PE3Component::shutdown() {
    ...
    bool PE3Component::isRunning() const {
        return m_isRunning.load(std::memory_order_acquire);
    }

    void PE3Component::reset() {
        // Reset del pipeline
        PC_F = 0x0;
        PCPlus8_F = 0x0;
    ...
    void PE3Component::stepUntil(int value) {
        if (!m_sharedData) {
            return;
        }
        if (value <= 0) value = 1;
    ...
    void PE3Component::stepIndefinitely() {
        if (!m_sharedData) {
            return;
        }
        auto& ctrl = m_sharedData->pe_control[m_pe_id];
    ...
    void PE3Component::stopExecution() {
        if (!m_sharedData) {
            return;
        }
        auto& ctrl = m_sharedData->pe_control[m_pe_id];
    ...
    void PE3Component::executeCycle() {
        auto& instConn = m_sharedData->instruction_connections[m_pe_id];
    ...
    void PE3Component::stageExecute() {
    ...
    void PE3Component::stageWriteBack() {
    ...
    void PE3Component::updateInstructionTracking() {
