...
namespace cpu_tlp {
    ...
    PE2Component::RegisterFile::RegisterFile() {
        reset();
    }

    void PE2Component::RegisterFile::reset() {
    ...
    uint64_t PE2Component::RegisterFile::read(uint8_t addr) const {
        if (addr >= 12) return 0ULL;
        return regs[addr];
    }

    void PE2Component::RegisterFile::write(uint8_t addr, uint64_t value, bool we) {
        if (!we) return;
        if (addr >= 12) return;
    ...
    PE2Component::ALU::Result PE2Component::ALU::execute(uint8_t control, uint64_t A, uint64_t B, uint8_t flagsIn) {
        Result res;
        res.value = 0;
        res.flags = flagsIn;
    ...
    PE2Component::ControlUnit::Signals PE2Component::ControlUnit::decode(uint8_t opcode) {
        Signals sig;
    ...
    PE2Component::HazardUnit::Outputs
        PE2Component::HazardUnit::detect(
            bool INS_READY,
    ...
    uint64_t PE2Component::extendImmediate(uint32_t imm, bool dataType) {
        if (dataType == 0) {
    ...
    bool PE2Component::evaluateBranchCondition(uint8_t branchOp, uint8_t flags) {
        bool N = (flags & 0x8) != 0;
        bool Z = (flags & 0x4) != 0;
        bool C = (flags & 0x2) != 0;
        bool V = (flags & 0x1) != 0;
    ...
    PE2Component::PE2Component(int pe_id)
        : m_pe_id(pe_id)
    ...
    PE2Component::~PE2Component() {
        shutdown();
    }

    bool PE2Component::initialize(std::shared_ptr<CPUSystemSharedData> sharedData) {
        if (m_isRunning.load()) {
            return false;
        }
        ...
        // Lanzar hilo
        m_executionThread = std::make_unique<std::thread>(&PE2Component::threadMain, this);

        std::cout << "[PE" << m_pe_id << "] Initialized successfully\n";
        return true;
    }

    void PE2Component::shutdown() {
    ...
    bool PE2Component::isRunning() const {
        return m_isRunning.load(std::memory_order_acquire);
    }

    void PE2Component::reset() {
        // Reset del pipeline
        PC_F = 0x0;
        PCPlus8_F = 0x0;
    ...
    void PE2Component::stepUntil(int value) {
        if (!m_sharedData) {
            return;
        }
        if (value <= 0) value = 1;
    ...
    void PE2Component::stepIndefinitely() {
        if (!m_sharedData) {
            return;
        }
        auto& ctrl = m_sharedData->pe_control[m_pe_id];
    ...
    void PE2Component::stopExecution() {
        if (!m_sharedData) {
            return;
        }
        auto& ctrl = m_sharedData->pe_control[m_pe_id];
    ...
    void PE2Component::executeCycle() {
        auto& instConn = m_sharedData->instruction_connections[m_pe_id];
    ...
    void PE2Component::stageExecute() {
    ...
    void PE2Component::stageWriteBack() {
    ...
    void PE2Component::updateInstructionTracking() {
