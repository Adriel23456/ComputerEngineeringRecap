...
void CacheTable::renderDataTable(const char* id, float tableHeight) {
    ...
    // 4 columns: Tag, Data(Hex), Decimal, Double
    if (ImGui::BeginTable(id, 4, flags, ImVec2(0, tableHeight))) {
        ...
        // Render 16 sub-rows for 16 x 8-byte segments
...
void CacheTable::render(const char* id) {
    ...
    // Navigation controls (no scrollbar)
    ImGuiWindowFlags ctrlFlags =
        ImGuiWindowFlags_NoScrollbar |
        ImGuiWindowFlags_NoScrollWithMouse;

    ImGui::BeginChild("##CacheControls", ImVec2(available.x, CTRL_HEIGHT), true, ctrlFlags);
    {
        ImGui::TextColored(ImVec4(0.4f, 0.8f, 1.0f, 1.0f),
