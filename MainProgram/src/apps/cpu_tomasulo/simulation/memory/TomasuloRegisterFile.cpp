// ============================================================================
// File: src/apps/cpu_tomasulo/simulation/memory/TomasuloRegisterFile.cpp
// ============================================================================
 ...
 *
 * Reset state:
 *   - All register values → 0 except LOWER → 0xFFFF…FFFF.
 *   - All Qi → 0, all Qi_valid → false.
 *
 * Named access (getByName / setByName) converts the name to uppercase
 * and delegates to nameToIndex(), throwing std::runtime_error on unknown names.
 */
...
TomasuloRegisterFile::TomasuloRegisterFile() { reset(); }
...
void TomasuloRegisterFile::set(int idx, uint64_t value) {
    if (idx >= 0 && idx < kCount) m_regs[idx].value = value;
}
...
TomasuloRegEntry& TomasuloRegisterFile::getEntry(int idx) {
    return m_regs[idx];  // caller is responsible for bounds check
}
