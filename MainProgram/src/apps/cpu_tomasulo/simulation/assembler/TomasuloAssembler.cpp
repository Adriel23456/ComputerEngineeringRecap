// ============================================================================
// File: src/apps/cpu_tomasulo/simulation/assembler/TomasuloAssembler.cpp
// ============================================================================

/**
 * @file TomasuloAssembler.cpp
 * @brief Implementation of the Tomasulo CPU two-pass assembler.
 *
 * Pass 1 — preprocessing:
 *   - Strips ; and # comments.
 *   - Extracts labels (colon-terminated identifiers) and records their PC.
 *   - Counts instructions so PC-relative branch offsets can be computed.
 *
 * Pass 2 — encoding:
 *   For each instruction line, identifies the format (R, I, branch, memory,
 *   unary, etc.) and encodes a 64-bit word:
 *     [63:56] opcode  [55:52] Rd  [51:48] Rn  [47:44] Rm
 *     [43:12] 32-bit immediate/offset   [11:0] reserved (zero)
 *
 *   Float immediates are encoded by reinterpreting the float bit pattern
 *   as uint32 (floatToU32).  Branch offsets are in bytes (PC × 8).
 *
 *   Any encoding error is collected per-line and reported in
 *   AssemblyResult::errors; the assembler continues to encode remaining
 *   lines so all errors are surfaced in a single pass.
 */
...
AssemblyResult TomasuloAssembler::assemble(const std::string& source) const {
    ...
    static const std::unordered_map<std::string, uint8_t> OPCODE_MAP = {
        ...
        {"MOVL",  0x33},  // load label address → reuses MOVI opcode
        // Integer comparisons
    ...
    std::vector<LineInfo> lines;
    {
        ...
        while (std::getline(stream, raw)) {
            ...
            if (!trim(raw).empty() && trim(raw)[0] == '#') raw.clear();
    ...
    for (auto& ln : lines) {
        ...
        if (!s.empty()) { instructions.emplace_back(pc, s, ln.lineNumber); ++pc; }
    }
    ...
    for (const auto& [currentPC, text, srcLine] : instructions) {
        try {
            ...
            // ── Memory instructions (bracket syntax) ──────────────
            if (opcVal == 0x4E || opcVal == 0x4F ||
                ...
                // R-format: Rn, Rm  or  Rd, Rm
                    ...
                    bool isCmp = (opcVal == 0x37 || opcVal == 0x38 || opcVal == 0x39 ||
                        opcVal == 0x3A || opcVal == 0x3F || opcVal == 0x40 ||
                        opcVal == 0x41);
                    if (isCmp) {
                ...
                // I-format: last operand is immediate
                    ...
                    bool isFPImm =
                        (opcVal >= 0x23 && opcVal <= 0x27) ||  // FADDI..FCOPYSIGNI
                    ...
                    uint32_t immU32 = 0;
                    if (isFPImm) {
                        float fv = std::stof(ops.back().substr(1));
                        immU32 = floatToU32(fv);
                    }
                    else {
                        immU32 = static_cast<uint32_t>(
                            static_cast<int32_t>(parseIntImm(ops.back())));
                    }
                ...
                // Branch: B<cc> <label>
                    ...
                    long     offsetInstr = static_cast<long>(labels.at(target)) - currentPC;
                    uint32_t offU32 = static_cast<uint32_t>(
                        static_cast<int32_t>(offsetInstr * 8));
